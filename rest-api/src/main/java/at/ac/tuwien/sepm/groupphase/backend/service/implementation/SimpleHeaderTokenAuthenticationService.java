...
public class SimpleHeaderTokenAuthenticationService implements HeaderTokenAuthenticationService {
    ...
    public AuthenticationToken authenticate(String username, CharSequence password) {
       ...
       at.ac.tuwien.sepm.groupphase.backend.entity.user.contacts.User user = (at.ac.tuwien.sepm.groupphase.backend.entity.user.contacts.User) userRepository.findOneByUsername(username).get();
        LoginUserResponseDTO loginR = new LoginUserResponseDTO();
        LOGGER.info("after getting user: "+username);
        LOGGER.info("user.getOnline_status(): "+user.getOnlineStatus());
        LOGGER.info("user.getProfilephoto(): "+user.getProfilePhoto());

          loginR.setId(user.getUserId());
          loginR.setFirstName(user.getFirstName());
          loginR.setLastNname(user.getLastName());
          loginR.setOnline_status(user.getOnlineStatus());
          loginR.setRole(user.getUserType().name());
         loginR.setProfilephoto(user.getProfilePhoto());
          loginR.setUsername(user.getUsername());
          loginR.setUserType(user.getUserType());
